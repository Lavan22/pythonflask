    ...
    if request.method == 'POST':
        try:
            ...
            with sqlite3.connect('database.db') as con:
                cur = con.cursor()
                cur.execute("INSERT INTO students (name, address, city, zip) VALUES (?,?,?,?)",(nm, addr, city, zip))
...
def editrec():
    ...
    if request.method == 'POST':
        try:
            ...
            with sqlite3.connect('database.db') as con:
                cur = con.cursor()
                cur.execute("UPDATE students SET name='"+nm+"', address='"+addr+"', city='"+city+"', zip='"+zip+"' WHERE rowid="+rowid)

                con.commit()
                msg = "Record successfully edited in the database"
        except:
            con.rollback()
            msg = "Error in the Edit: UPDATE students SET name="+nm+", address="+addr+", city="+city+", zip="+zip+" WHERE rowid="+rowid
